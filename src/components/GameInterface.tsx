...
function AgentConversationInterface({ serviceType }: { serviceType: ServiceType }) {
  ...
  const { messages, isLoading, selectedClient, isInputDisabled, setSelectedClient, resetConversation, waitingForNoResponse, input, handleInputChange, handleSubmit } = useAgentConversation('agent-resolutor', undefined, mode);
  
  const CLIENT_TYPES: { value: ServiceType; label: string; description: string }[] = [
    { value: "client-claim", label: "Cliente Normal", description: "Presenta un problema de manera calmada" },
    { value: "desperate-client", label: "Cliente Desesperado", description: "Tiene urgencia y necesita ayuda inmediata" },
    { value: "angry-client", label: "Cliente Enojado", description: "Está frustrado y puede ser confrontativo" },
  ];

  return (
    <div className="flex h-full overflow-hidden">
      {/* Panel de Configuración - Lado Izquierdo FIJO */}
      <div className="w-80 bg-gray-50 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 p-6 space-y-6">
          ...
          <div>
            <label htmlFor="client-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
              Tipo de Cliente
            </label>
            <select
              id="client-select"
            ...
            {selectedClient && (
              <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                  {CLIENT_TYPES.find((c) => c.value === selectedClient)?.description}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              {selectedClient && (
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  {CLIENT_TYPES.find((c) => c.value === selectedClient)?.label} • Modo {mode === 'automatic' ? 'Automático' : 'Personalizado'}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Elige un tipo de cliente en el panel izquierdo para comenzar la conversación
                </p>
              </div>
            </div>
          ) : (
              ...
              {waitingForNoResponse && (
                <div className="bg-yellow-50 dark:bg-yellow-900/20 px-4 py-2 text-sm text-center border-b border-yellow-200 dark:border-yellow-800 flex-shrink-0">
                  <span className="text-yellow-800 dark:text-yellow-200">
                    ⏳ Cliente no responde... cerrando conversación profesionalmente
                  </span>
                </div>
              )}
